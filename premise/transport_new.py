...
def _update_transport(scenario, version, system_model):
    ...
    if (
        scenario["iam data"].roadfreight_markets is not None
        and scenario["iam data"].railfreight_markets is not None
    ):
        ...
        scenario["iam data"].roadfreight_markets is not None
        and scenario["iam data"].railfreight_markets is None
    ):
        print("No railfreight markets found in IAM data. Skipping freight transport.")

    elif (
        scenario["iam data"].roadfreight_markets is None
        and scenario["iam data"].railfreight_markets is not None
    ):
...
class Transport(BaseTransformation):
    ...
    def generate_datasets(self):
        ...
        roadfreight_dataset_names = self.iam_data.roadfreight_markets.coords[
            "variables"
        ].values.tolist()
        railfreight_dataset_names = self.iam_data.railfreight_markets.coords[
            "variables"
        ].values.tolist()
        freight_transport_dataset_names = (
            roadfreight_dataset_names + railfreight_dataset_names
        )
        ...
        for dataset in self.database:
            ...
            if dataset["name"] in railfreight_dataset_names:
                if dataset["location"] != "RoW":
                    ...
                    changed_datasets_location.append(
        ...
        for region in self.iam_data.regions:
            for freight_transport_ds in freight_transport_dataset_names:
                if [
                    freight_transport_ds,
                    region,
                ] not in changed_datasets_location and region != "World":
                    try:  # RoW dataset to be used for other IAM regions
                        new_dataset = copy.deepcopy(
                            ws.get_one(
                                self.database,
                    ...
                    if not any(
                        ...
                        and dataset["location"] == new_dataset["location"]
                    ...
                    new_dataset["comment"] = (
                        f"Dataset for the region {region}. {new_dataset['comment']}"
                    )
        ...
        for dataset in list(self.database):  # Create a copy for iteration
            if any(
                ...
                and old_dataset["location"] == dataset["location"]
        ...
        if scaling_factor != 1 and scaling_factor > 0:
            rescale_exchanges(
                ...
                technosphere_filters=[
                    ws.either(
                        *[ws.contains("name", x) for x in energy_carriers]
                    )  # TODO: apply diesel efficiency increase to diesel shunting for electricity and hydrogen datasets
                ],
                biosphere_filters=[
                    ws.contains("name", x) for x in fuel_combustion_emissions
                ],
            ...
            dataset["comment"] = text + (
                dataset["comment"] if dataset["comment"] is not None else ""
            )
    ...
    def generate_transport_markets(self):
        ...
        transport_markets_tbc = {
            "market for transport, freight, lorry, unspecified powertrain": self.iam_data.roadfreight_markets.coords[
                "variables"
            ].values.tolist(),
            "market for transport, freight train, unspecified powertrain": self.iam_data.railfreight_markets.coords[
                "variables"
            ].values.tolist(),
        }
                ...
                if region != "World":
                    for vehicle in vehicles:
                        if (
                            ...
                            market_share = self.iam_data.roadfreight_markets.sel(
                                region=region, variables=vehicle, year=self.year
                            ).item()
                        elif (
                            markets
                            == "market for transport, freight train, unspecified powertrain"
                        ):
                            market_share = self.iam_data.railfreight_markets.sel(
                                region=region, variables=vehicle, year=self.year
                            ).item()
        ...
        vehicles_map = get_vehicles_mapping()
        dict_transport_ES_var = vehicles_map["energy service variables"][self.model][
            "mode"
        ]
                ...
                if region != "World":
                    ...
                    dict_regional_shares[region] = (
                        self.iam_data.data.sel(
                            region=region, variables=var, year=self.year
                        ).values
                    ) / (
                        self.iam_data.data.sel(
                            region="World", variables=var, year=self.year
                        ).item()
                    )
        ...
        for dataset in self.database:
            if "transport, freight" not in dataset["name"]:
                for exc in ws.technosphere(
                    ...
                    if any(
                        key.lower() in exc["name"].lower()
                        for key in vehicles_map["freight transport"][self.model]
                    ):
                        key = [
                            k
                        ...
                        exc["name"] = (
                            f"{vehicles_map['freight transport'][self.model][key]}"
                        )
    ...
    def delete_inventory_datasets(self):
        ...
        self.database = [ds for ds in self.database if ds["name"] not in ds_to_delete]
